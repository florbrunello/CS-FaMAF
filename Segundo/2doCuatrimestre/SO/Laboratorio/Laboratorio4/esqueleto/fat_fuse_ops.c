...
static void fat_fuse_log_activity(char *operation_type, fat_file file) {
    ...
    char* path = BB_LOG_FILE; 
    char* write="write"; 
    char* read="read";
    fat_volume vol = get_fat_volume(); 
    fat_tree tree = vol->file_tree; 
    fat_tree_node file_node = fat_tree_node_search(tree,path); 
    fat_file parent = fat_tree_get_parent(file_node);
    fat_file fs_log = fat_tree_search(tree,path);

    //Escribo en el archivo actividad del usuario
    if((operation_type==read)||(operation_type==write)){
        fat_file_pwrite(fs_log,buf,message_size*sizeof(char),fs_log->dentry->file_size,parent); 
    }
 }


static void fat_fuse_create_log_file(void){
    char * path = BB_LOG_FILE;

    fat_volume vol = get_fat_volume();
    fat_file log = fat_tree_search(vol->file_tree,path); 

    if (log!=NULL){ //Si el archivo está creado
        printf("Log already exists\n");
        return;
    }
 
    int mknod_exit = fat_fuse_mknod(path, 0, 0);
    if((mknod_exit==ENOENT)||(mknod_exit==ENOTDIR)){ //Chequeo de error en la función fat_tree_search
        printf("Unable to create file\n");
        return; 
    }

    //Actulizamos el disco
    fat_file parent = fat_tree_search(vol->file_tree,BB_DIRNAME);
    fat_file_dentry_add_child(parent,log);
...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    if (!fat_tree_search(vol->file_tree,BB_DIRNAME)){
        bb_create_new_orphan_dir(); 
        if (!fat_tree_search(vol->file_tree,BB_LOG_FILE)){
            fat_fuse_create_log_file(); 
        }
    }
}
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    char* dir_path = BB_DIRNAME;

    fat_volume vol = get_fat_volume(); 
    fat_tree tree = vol->file_tree; 
    fat_file bb_dir = fat_tree_search(tree,dir_path);

    while (*child != NULL){
        if(bb_dir->name!=(*child)->name){
...
int fat_fuse_rmdir(const char *path) {
    ...
    //Obtenemos el directorio que seamos borrar
    file = fat_tree_get_file(file_node);

    //Chequeamos que es realmente un directorio
    if (!fat_file_is_directory(file)) {
        errno = ENOTDIR;
        return -errno;
    }

    //Chequeamos que es un directorio vacio
    fat_file* file_childrens = fat_tree_flatten_h_children(file_node);
    if (*file_childrens != NULL)
    {
        fat_error("Is not an empty directory\n");
        errno = EPERM;
        return -errno;
    }
    ...
    parent = fat_tree_get_parent(file_node);
    fat_tree_inc_num_times_opened(file_node);
    //Liberamos el cluster y actualizamos la entrada del directorio
    fat_file_rmdir(file, parent);

    //Actualizamos el arbol de directorios
...
int fat_fuse_unlink(const char *path) {
    ...
    //Obtenemos el archivo que seamos borrar
    file = fat_tree_get_file(file_node);

    //Chequeamos que es realmente un archivo
    if (fat_file_is_directory(file)) {
        errno = EISDIR;
        return -errno;
    }
    ...
    parent = fat_tree_get_parent(file_node);
    fat_tree_inc_num_times_opened(file_node);
    //Liberamos los clusters y actualizamos la entrada del archivo
