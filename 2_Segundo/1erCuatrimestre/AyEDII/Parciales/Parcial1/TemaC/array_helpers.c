/*
@file array_helpers.c
@brief Array Helpers method implementation
*/
#include <assert.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>

#include "array_helpers.h"

/**
* @brief returns true when reach last entry in flight table
* @return True when is the last entre of the flight table, False otherwise
*/
static bool is_last_line(unsigned int hour, unsigned int type) {
  return  hour == HOURS - 1u && type == TYPE - 1u;
}

void array_dump(DeliveryTable a) {
  for (unsigned int type = 0u; type < TYPE; ++type) {
    for (unsigned int hour = 0u; hour < HOURS; ++hour) {
      Flight f = a[type][hour];
      fprintf(stdout, "%c: flight with %u %s arrived at %u:00", f.code, f.items_amount, f.type == 0 ? "boxes" : "letters", f.hour - 1);
      if (!is_last_line(hour, type))
      {
        fprintf(stdout, "\n");
      }
    }
  }
}

unsigned int extra_space_fee_cost (DeliveryTable a, unsigned int h) {
  
  unsigned int fee=0u; 
  unsigned int sum_boxes = 0u;
  unsigned int sum_letters = 0u;

  /*Calculo la cantidad de cartas y cajas que llegaron hasta las 20 hs INCLUSIVE*/
  for(unsigned int i=0u; i<=h; i++){     // < h 
    sum_boxes += a[boxes][i].items_amount;
    sum_letters += a[letters][i].items_amount;
  }
  
  /*Calculo el impuesto por cada caja y carta extra que se almaceno y lo sumo*/
  if (sum_boxes > MAX_ALLOWED_BOXES){
    fee += (sum_boxes - MAX_ALLOWED_BOXES) * BOX_PENALTY;
  }

  if (sum_letters > MAX_ALLOWED_LETTERS){
    fee += (sum_letters - MAX_ALLOWED_LETTERS) * LETTER_PENALTY;
  }

  return fee;
  
}

void array_from_file(DeliveryTable array, const char *filepath) {
  FILE *file = NULL;

  file = fopen(filepath, "r");
  if (file == NULL) {
    fprintf(stderr, "File does not exist.\n");
    exit(EXIT_FAILURE);
  }

  char code;
  unsigned int arrival_hour;
  int i = 0;

  /*Leo hasta que termina de leer el archivo o me exedo de las horas*/

  while (!feof(file) && i<HOURS/*lectura completa de todos los datos */) {
    int res = fscanf(file,"\n_%c_ ",&code/*lectura de codigo de vuelo */);
    if (res != 1) {
      fprintf(stderr, "Invalid file.\n");
      exit(EXIT_FAILURE);
    }
    int readed = fscanf(file," %u ",&arrival_hour/*lectura de hora de llegada */);  // " %u " o "%u"
    if (readed != 1) { 
      fprintf(stderr, "Invalid file.\n");
      exit(EXIT_FAILURE);
    }

    /*Chequeo que el formato de hora este bien en el archivo*/
    if (arrival_hour > HOURS) 
    {
      fprintf(stderr,"Invalid file");
      exit(EXIT_FAILURE);
    }

    /* COMPLETAR: Generar y guardar ambos Flight en el array multidimensional */
    Flight flight_boxes = flight_from_file(file,code,arrival_hour);/* completar... */
    Flight flight_letters = flight_from_file(file,code,arrival_hour);/* completar... */

    array[flight_boxes.type][flight_boxes.hour-1u] = flight_boxes;
    array[flight_letters.type][flight_letters.hour-1u] = flight_letters;

    i++;
  }
}
