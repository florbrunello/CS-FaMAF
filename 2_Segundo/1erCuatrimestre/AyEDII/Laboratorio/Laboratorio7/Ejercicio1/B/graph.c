...
graph_t graph_empty(unsigned int max_vertexs)
{
    ...
    cost_t **costs_array = calloc(max_vertexs,sizeof(cost_t *));  //OJO size of
    /*
     * TODO: COMPLETAR.
     */
    for (unsigned int i = 0u; i < graph->max_vertexs; i++) {
        costs_array[i] = calloc(max_vertexs,sizeof(cost_t));
        for (unsigned int j = 0u; j < graph->max_vertexs; j++) {
            costs_array[i][j] = cost_inf();
        }
    }
